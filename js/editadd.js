...
const buildMatchemPreview = (target, data, archive) => {
  ...
  if (archive) {
    document.getElementById(target).innerHTML +=
      "<div id = 'send_to_archive'> <i class='fas fa-archive'></i> </div>";
    document
      .getElementById("send_to_archive")
...
function setChanges() {
  ...
  document.getElementById("set_select").innerHTML +=
    '<option value="" disabled selected hidden>Choose a flash card set...</option>';
  for (i = 0; i < sets.length; i++) {
    document.getElementById("set_select").innerHTML +=
      `<option value="${sets[i]}"> ${sets[i]} (${scores[sets[i]]})</option>`;
  }
...
function removeMode() {
  if (removeFlag == 0) {
    ...
    els = document
      .getElementById("demo_zone")
      .getElementsByClassName("matchem_row");
...
addButton.addEventListener("pointerdown", () => {
  if (selectedSet) {
    ...
    document.getElementById("set_name").value
      ? document.getElementById("set_name").value
      : window.alert("name your set to make changes");
  }
});
setChanges();
